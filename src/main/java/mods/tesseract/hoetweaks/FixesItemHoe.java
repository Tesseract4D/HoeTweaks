...
public class FixesItemHoe {
    ...
    public static float func_150893_a(ItemHoe c, ItemStack k, Block b) {
        ...
        return (m == Material.grass || m == Material.ground || m == Material.plants || m == Material.cactus || m == Material.leaves || m == Material.vine || m == Material.sponge) ? c.theToolMaterial.getEfficiencyOnProperMaterial() : 1;
    }

    @Fix(returnSetting = EnumReturnSetting.ON_TRUE, booleanAlwaysReturned = false)
    public static boolean onItemUse(ItemHoe c, ItemStack item, EntityPlayer p, World world, int x, int y, int z, int side, float i, float j, float k) {
        return side != -1;
    }
}
